...
for i=1:n
    % v = i;
    Degrees(i) = sum(E(i,:) == 1); % size([E(E(:,1)==v,2); E(E(:,2)==v,1)],1);
end
...
for i=1:n
    if i == 1
        ...
        neighbors = E(v, :) == 1; % [E(find(E(:,1)==v),2); E(find(E(:,2)==v),1)];
    ...
    neighbors_v = E(v, :) == 1; % [E(find(E(:,1)==v),2); E(find(E(:,2)==v),1)];
    for j=1:size(neighbors_v,1)
       u = neighbors_v(j);
       neighbors_u = E(u, :) == 1; % [E(find(E(:,1)==u),2); E(find(E(:,2)==u),1)];
